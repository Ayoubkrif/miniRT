...
/*   Updated: 2025/07/02 14:56:50 by aykrifa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_rgb	is_it_touching(t_rt *rt, double x, double y)
{
	...
	phong.ray_light_obj_n = normalize(vec_sub(phong.point_cam, rt->light.position));
	...
	while (1)
	{
		...
		phong.point_light = get_point(rt->light.position, phong.ray_light_obj_n, phong.inter_light.t);
